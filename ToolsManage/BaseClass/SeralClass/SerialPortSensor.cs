...
namespace ToolsManage.BaseClass
{
    ...
    public class SerialPortSensor
    {
        ...
        public SerialPortSensor(string name, SerialPortBaudRates baud, Parity par, SerialPortDatabits dBits, StopBits sBits)
        {
            _portName = name;
            _baudRate = baud;
            _parity = par;
        ...
        /// <param name="dBits">数据位</param>
        /// <param name="name">串口号</param>
        public SerialPortSensor(string name, string baud, string par, string dBits, string sBits)
        {
            _portName = name;
        ...
        ///// <returns></returns>
        //public int SendReceByte(byte[] SendData, ref  byte[] ReceiveData, int Overtime)
        //{
        //    int ret = 0;
        //    try
        //    {
        ...
        public int SendReceByte(byte[] SendData, ref  byte[] ReceiveData, int Overtime, IsWait isWait)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < Overtime; i++)
                {
                    ...
                    {
                        if (clsEnvirControl.blAskSensor)
                            Thread.Sleep(100);
                        else
                            i = Overtime;
                    }
